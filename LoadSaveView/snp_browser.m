function snp_browser(snp)
%takes the output of trig2snps (or several, grouped in a 3D-cellarray{siteID,tp}
%and allows to browse through individual onsets
%2019 FW

images=[];
if isa(snp,'cell')
    for siteID=1:size(snp,1)
        for tp=1:size(snp,2)
            s=snp{siteID,tp};
            if isempty(snp),continue; end
            for ind=1:size(s,3)
                figure(9850);clf;
                imagesc(s(:,:,ind))
                colormap gray;
                hold on;
                xline(100,'LineWidth',3,'LineStyle',':','Color','red');
                caxis([0.6 1.5])
                annotation('textbox', [.7,1,0,0], 'string', sprintf('siteID|tp|onset::[%i|%i|#%i]',siteID,tp,ind))
                images=cat(4,images,getfield(getframe(figure(9850)),'cdata'));
            end
        end
    end
else
    for ind=1:size(snp,3)
        figure(9850);clf;
        imagesc(snp(:,:,ind))
        colormap gray;
        hold on;
        xline(100,'LineWidth',3,'LineStyle',':','Color','red');
        caxis([0.6 1.5])
        annotation('textbox', [.7,1,0,0], 'string', sprintf('onset::[#%i]',ind))
        images=cat(4,images,getfield(getframe(figure(9850)),'cdata'));
    end
end
view_plots(images)
    ...
    function view_plots(in,varargin)
        % attempts to convert open figures into a 'movie', displayed by view_stack.
        % basic idea is the quick&dirty visualization changes over time e.g.
        % between timepoints
        % closes open figures
        % FW 2018
        ...
        if ~exist('in','var')||isempty(in)
            data=[];
            ffs=fliplr(findall(0,'type','figure')');
            
            [~,idx]=sort([ffs.Number]);
            ffs=ffs(idx);
            ffs(find(strcmp('calliope',{ffs.Name})))=[];
            ...
            for ind=[ffs.Number]
                ...
                figure(ffs(ind))
                hhh=handle(ffs(ind));
                if isempty(hhh.WindowButtonDownFcn) %detect view_stack windows, exclude from datacollection
                    data=cat(4,data,frame2im(getframe(gcf)));
                end
            end
            ca; %close all figures
        else
            data=in;
        end
        ...
        template=data(:,:,:,round(size(data,4)/2));
        ...
        params.h_im_data=imshow(data(:,:,:,frame_ind));
        ...
        switch event.Character
            ...
            case 'm' % make a movie
                ...
                for cp=frame_bounds(1)/nFrames:spacing*speed_factor:(frame_bounds(2)+1)/nFrames-spacing*speed_factor
                    ...
                    set(params.h_im_data,'CData',mean(data(:,:,:,curr_frame_ind:min(size(data,4),curr_frame_ind-1+speed_factor+params.nbr_avg_frames)),4));
            ...
            case 'c' % clear
                ...
                cp=cp(1);
                axes(params.h_f_ax);
                params.h_im_data=imshow(data(:,:,round(cp*(size(data,4)-1))+1));
                set(params.h_f_ax,'clim',params.clim,'xtick',[]);
                yl=ylim;xl=xlim;
                params.h_txt=text(xl(2)/20,yl(2)/20,num2str(round(cp*(size(data,4)-1))+1),'fontsize',12,'color','w','fontweight','bold');
                if isfield(params,'sup_fig_h') && ishandle(params.sup_fig_h)
            ...
            case 'r' % compare first and last frames
                ...
                axes('position',[0 0 1 1]);
                %         [shift_x,shift_y] = register_frames(data(:,:,[1:params.nbr_insp_frames end-params.nbr_insp_frames:end]),mean(data(:,:,10:20),4),0.2);
                [shift_x,shift_y] = register_frames(data(:,:,[1:params.nbr_insp_frames 100:100+params.nbr_insp_frames]),mean(data(:,:,10:20),4),0.2);
                ...
                imshowpair(mean(tempdata(:,:,1:params.nbr_insp_frames),4), mean(tempdata(:,:,params.nbr_insp_frames+1:end),4),'montage')
                set(gca,'clim',params.clim)
                axis off
                figure(prev_fig_handle);
                set(hf,'UserData',params);
    ...
    function draw_frame(cp,h_sl_bar,params,data)
        curr_frame_ind=round(cp*(size(data,4)-1))+1;
        ...
        if params.nbr_avg_frames==1
            ...
            if params.tx~=0 || params.ty~=0
                ...
                frame=mean(data(:,:,:,curr_frame_ind:min(curr_frame_ind+params.nbr_avg_frames-1,size(data,4))),4);
            end
            set(params.h_im_data,'CData',frame);
