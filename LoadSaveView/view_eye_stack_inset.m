function view_eye_stack_inset(data, imeta_info, roi_act, frame_times, iframe_times )
...
% inset=zeros(10,10,length(roi_act));
% for ind=1:length(roi_act)
%     inset(:,:,ind)=roi_act(ind);
% end

for ind=1:length(iframe_times)
    [~,knd]=min(abs(iframe_times(ind)-frame_times));
    inset_ind(ind)=knd;
end

aa=roi_act(inset_ind);
% inset=double(inset);
% data(end-size(inset,1)+1:end,end-size(inset,2)+1:end,:)=uint8(inset(:,:,inset_ind)/max(inset(:))*double(max(data(:))));

NOP=100;
THETA=linspace(0,2*pi,NOP);
pupil=zeros(NOP,2,size(imeta_info,2));
for ind=1:length(imeta_info)
    RHO=ones(1,NOP)*aa(ind)/10;%imeta_info(9,ind);
...
yl=ylim;xl=xlim;
h_txt=text(xl(2)/20,yl(2)/20,'1','fontsize',12,'color','w','fontweight','bold');
colormap(map);
