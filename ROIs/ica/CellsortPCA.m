function [mixedsig, mixedfilters, CovEvals, covtrace, movm, ...
    movtm] = CellsortPCA(data, flims, nPCs, dsamp, outputdir, badframes)
...
[pixw,pixh,~] = size(data);%size(imread(fn,1));
npix = pixw*pixh;
...
firstframe_full = data(:,:,1);%imread(fn,1);

% Create covariance matrix
if nt < npix
    [covmat, mov, movm, movtm, pixw, pixh] = create_tcov(data, pixw, pixh, useframes, nt, dsamp);
else
    [covmat, mov, movm, movtm] = create_xcov(data, pixw, pixh, useframes, nt, dsamp);
end
clear data
npix = pixw* pixh;
...
toc
    ...
    function [covmat, mov, movm, movtm] = create_xcov(data, pixw, pixh, useframes, nt, dsamp)
        %-----------------------
        ...
        if (dsamp_space==1)
            ...
            [pixw_dsamp,pixh_dsamp] = size(imresize( data(:,:,1), 1/dsamp_space, 'bilinear' ));
            mov = zeros(pixw_dsamp, pixh_dsamp, nt);
            for jjind=1:length(useframes)
                jj = useframes(jjind);
                %                 mov(:,:,jjind) = imresize( imread(fn,jj), 1/dsamp_space, 'bilinear' );
                mov(:,:,jjind) = imresize( data(:,:,jj), 1/dsamp_space, 'bilinear' );
                if mod(jjind,500)==1
%                     fprintf(' Read frame %4.0f out of %4.0f; ', jjind, nt)
                    toc
                end
...
%         fprintf(' Read frame %4.0f out of %4.0f; ', jjind, nt)
        clear data
        mov = reshape(mov, pixw_dsamp*pixh_dsamp, nt);
        ...
        mov(movmzero, :) = 0;
        if dsamp_time>1
            mov = filter(ones(dsamp,1)/dsamp, 1, mov, [], 2); % calculate moving average
    ...
    function [covmat, mov, movm, movtm, pixw, pixh] = create_tcov(data, pixw, pixh, useframes, nt, dsamp)
        %-----------------------
...
%             [pixw_dsamp,pixh_dsamp] = size(imresize( imread(fn,1), 1/dsamp_space, 'bilinear' ));
            [pixw,pixh] = size(imresize( data(:,:,1), 1/dsamp_space, 'bilinear' ));
            mov = zeros(pixw, pixh, nt);
            npix = pixw*pixh;
            for jjind=1:length(useframes)
                jj = useframes(jjind);
...
%         fprintf(' Read frame %4.0f out of %4.0f; ', jjind, nt)
        ...
        if dsamp_time>1
            mov = filter(ones(dsamp(1),1)/dsamp(1), 1, mov, [], 2);
            mov = mov(:,1:dsamp(1):end);
            %mov = downsample(mov', dsamp(1))';
        end
        toc
        c1 = (mov'*mov)/npix;
